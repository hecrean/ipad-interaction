...
export class LRUCache<TKey = string, TValue = any>
  implements Iterable<LRUCacheEntry<TKey, TValue>>
{
  ...
  private readonly onEntryEvicted?: (evictedEntry: {
    key: TKey;
    value: TValue;
    isExpired: boolean;
  }) => void;

  private readonly onEntryMarkedAsMostRecentlyUsed?: (entry: {
    key: TKey;
    value: TValue;
  }) => void;
  ...
  public set(
    key: TKey,
    value: TValue,
    entryOptions?: LRUCacheSetEntryOptions<TKey, TValue>
  ...
  ): LRUCacheEntry<TKey, TValue> | null {
    let node = this.head;
  ...
  private mapNodeToEntry({
    key,
    value,
  }: LRUCacheNode<TKey, TValue>): LRUCacheEntry<TKey, TValue> {
    return {
      key,
      value,
    };
